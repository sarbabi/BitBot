...
def send_orders(bid, ask, maker_commission, strategy, volume=0.001):
...
def send_order(strategy, side, price, maker_commission=0.001, volume=0.001):
    ...
    if side == 'buy':
        ...
        if price * (1 + maker_commission) <= cash:
            ...
            status = 'error'
        if localsettings.TEST_MODE:
            buy_order = Orders(side='buy', price=price,net_price=round(price*(1+maker_commission),2), status=status, volume=volume, insert_time=get_now(),
                               strategy_id=strategy.id)
            session.add(buy_order)
